        ...
        for sub_counter = 1:length(subj_list)
            ...
            for iFpass = 1:size(fpass_list,1)
                ...
                if strcmp('', lag)
                    ...
                    ch1_to_ch2 =nan(1,length(reg1) * length(length(reg1)+1:length(allreg)));
                    ch2_to_ch1 =nan(1,length(reg1) * length(length(reg1)+1:length(allreg)));
                ...
                for delta =  delta_val
                    ...
                    for iElec1 = 1:length(reg1) % Reg1 elecs
                        for iElec2 = length(reg1)+1:length(allreg) % Reg2 elecs
                            cntr = cntr+1;
                            str = sprintf('%d%d', iElec1, iElec2-length(reg1));
                            elecPair (cntr) = str2double(str);
                            if ismember (allreg (iElec2),CA1_chan_idx ) % CA1
                            reg2Code (cntr) = 1;
                            elseif ismember (allreg (iElec2),CA3_chan_idx ) % CA3
                             reg2Code (cntr) = 2;   
                            else % Neither CA1 nor CA3 but HC
                             reg2Code (cntr) = 3;   
                            end
                            ...
                            ch1_bin_width = 3.5*circ_std(ch1_phase(:))/length(ch1_phase(:))^(1/3); %  used in the original PTE paper.
                            ...
                            ch2_ch1_crnt_phase_pairs = [reshape(ch2_phase(1:end-delta,:),numel(ch2_phase(1:end-delta,:)),1), ...
                                reshape(ch1_phase(1:end-delta,:),numel(ch1_phase(1:end-delta,:)),1)];
                            ...
                            x1pst  = reshape(ch1_phase(1:end-delta,:),numel(ch1_phase(1:end-delta,:)),1);
                            ...
                            if strcmp('', lag)
                                ch1_to_ch2 (cntr) = PTE_ch1_to_ch2a ;
                                ch2_to_ch1 (cntr) = PTE_ch1_to_ch2b ;
                                PTE_ch1_to_ch2_norm (cntr) = ch1_to_ch2 (cntr) / (ch1_to_ch2 (cntr) + ch2_to_ch1 (cntr));
                            else
                ...
                if strcmp('',lag)
                save([reg1_name 'vs' reg2_name '_subj'  subj_list{sub_counter} '_cond' num2str(cond_num) sig_elecs],...
                    'PTE_ch1_to_ch2_norm', 'ch1_to_ch2', 'ch2_to_ch1', 'delta_val','elecPair','reg2Code')
                else
                save([reg1_name 'vs' reg2_name '_subj'  subj_list{sub_counter} '_cond' num2str(cond_num) sig_elecs '_FOTofLag'],...
                    'PTE_ch1_to_ch2_norm', 'ch1_to_ch2', 'ch2_to_ch1', 'delta_val','elecPair','reg2Code')
