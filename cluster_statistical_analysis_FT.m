...
load(['subj' subj 'spectrograms.mat'])
...
% rearrange data
if strcmp('OFC',reg)
    conda=OFC_cond2(:,301:1301,:);
    condb=OFC_cond3(:,301:1301,:);
elseif strcmp('FRO',reg)
    conda=fro_cond2(:,301:1301,:);
    condb=fro_cond3(:,301:1301,:);
elseif strcmp('TEMP',reg)
    conda=temp_cond2(:,301:1301,:);
    condb=temp_cond3(:,301:1301,:);
elseif strcmp('MTL',reg)
    conda=MTL_cond2(:,301:1301,:);
    condb=MTL_cond3(:,301:1301,:);
end
...
wDataCond2.time   = linspace(-pre_stim+(edge_points/fs),post_stim-(edge_points/fs),size(cond2,3));
wDataCond2.cfg    = [];
...
wDataCond3.time   = linspace(-pre_stim+(edge_points/fs),post_stim-(edge_points/fs),size(cond3,3));
wDataCond3.cfg    = [];
